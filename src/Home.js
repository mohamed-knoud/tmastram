    ...
    <div>
        <div id="wrapper">
            {actual && <img id="poster" src={`https://image.tmdb.org/t/p/original${actual.poster_path}`} alt=""/>}<div>
                ...
                <i style={{fontSize:'0.9em',margin:'0 10px',cursor:'pointer'}} className="fa-solid fa-plus"></i>

                </Tooltip>
                <Tooltip text="Share">
                <i style={{fontSize:'0.9em',margin:'0 10px',cursor:'pointer'}} className="fa-solid fa-share"></i>
...
{window.innerWidth>800 && <div style={{color:'white',textAlign:'right',position:'relative',bottom:'20px',right:'50px'}}>
  ...
  {actual4 !== null && actual4.map(movie => (
    <div style={{display:'flex',flexDirection:'column',marginLeft:'10px'}}  key={movie.id}> {/* Remember to add a unique key prop when iterating over lists */}
      ...
      <div style={{width:'100%',backgroundColor:'#2f2f2F',textAlign:'center',color:'#a4b2c8'}}>
     <Link to=""> تنبيه جميع الافلام والمسلسلات المتوفرة على منصتنا محملة من مواقع الانترنت لمشاهدة الافلام والمسلسلات بشكل افضل على المنصات الرسمية </link>اضغط هنا
